...
| Strategy enum | `Classes/Integration/IntegrationStrategy.php` | `DiClassReplacement`, `ProviderConfiguration` (ADR-001 designs four; cases are added with their first consumer), `ToolProvision` (ADR-002: a tool shipped on behalf of an extension that has no nr-llm writer) |
...
- ADR-002 — A writer for EXT:news, shipped as a tool: `Documentation/Adr/Adr002NewsDraftToolProvision.rst` (the tool-provision strategy, what the tool fixes, why the writers' traits are not reused).
