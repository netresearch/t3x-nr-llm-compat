...
final class ThirdPartyCompatibilityPassTest extends UnitTestCase
{
    ...
    #[Test]
    public function activeToolProvisionIntegrationRegistersTheToolUnderTheNrLlmToolTag(): void
    {
        ...
        self::assertTrue($container->hasDefinition(FixtureContractImplementation::class));
        $definition = $container->getDefinition(FixtureContractImplementation::class);
        // The tag is what nr-llm's ToolRegistry collects; without it the
        // service exists and the tool is never offered.
        self::assertTrue($definition->hasTag('nr_llm.tool'));
        self::assertTrue($definition->isAutowired());
    }

    #[Test]
    public function disabledToolProvisionIntegrationRegistersNoTool(): void
    {
        $integration = new ConfigurableToolIntegration(packageName: self::INSTALLED_PACKAGE);
        $container = new ContainerBuilder();
